...
/*!
 * \file
 * \brief Plik zawiera definicje metod klasy XMLInterp4Config
...
//Tutaj odbywa sie wczytywanie wartosci do strumienia
 IStrm >> x >> y >> z;
 if (IStrm.fail()) {
     cerr << " Blad wczytywania" << endl;
 } else {
     cout << " Prawidłowe odczytywanie wartosci" << endl;
     cout << "     " << x << "  " << y << "  " << z << endl;
 }
